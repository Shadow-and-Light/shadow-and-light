...
function S:Setup()
	...
	--Positioning
	SS.AFKtitle:SetPoint("TOP", SS.Top, "TOP", -25, -10)
	SS.Subtitle:SetPoint("TOP", SS.AFKtitle, "BOTTOM", 25, -2)
	SS.timePassed:ClearAllPoints()
	SS.timePassed:SetPoint("LEFT", SS.AFKtitle, "RIGHT", 4, -1)
	SS.ExPack:SetPoint("CENTER", SS.Top, "BOTTOM", 0, 0)
	SS.Time:SetPoint("TOP", SS.Date, "BOTTOM", 0, -2)
	...
	SS.PlayerName:SetPoint("BOTTOM", SS.PlayerInfo, "TOP", 0, 2)
...
function S:Show()
	...
	SS.FactCrest:SetPoint("CENTER", SS.Top, "BOTTOM", -(GetScreenWidth()/6) + S.db.crest.xOffset_faction, 0 + S.db.crest.yOffset_faction)
	SS.RaceCrest:SetPoint("CENTER", SS.Top, "BOTTOM", (GetScreenWidth()/6) + S.db.crest.xOffset_race, 0 + S.db.crest.yOffset_race)
	SS.Date:SetPoint("RIGHT", SS.Top, "RIGHT", -40 + S.db.date.xOffset, 10 + S.db.date.yOffset)
	SS.PlayerInfo:SetPoint("RIGHT", SS.TOP, "RIGHT", -(GetScreenWidth()/6), 0)
...
function S:SetupType()
	...
	if S.db.animTime > 0 then
		if S.db.animType == "SlideIn" then
			SS.Top:SetPoint("BOTTOM", SS, "TOP", 0, E.Border)
			SS.Bottom:SetPoint("TOP", SS, "BOTTOM", 0, - E.Border)
			SS.Top:SetAlpha(1)
			SS.Bottom:SetAlpha(1)
		elseif S.db.animType == "SlideSide" then
			SS.Top:SetPoint("TOPRIGHT", SS, "TOPLEFT", 0, E.Border)
			SS.Bottom:SetPoint("BOTTOMLEFT", SS, "BOTTOMRIGHT", 0, - E.Border)
			SS.Top:SetAlpha(1)
			SS.Bottom:SetAlpha(1)
		else
			SS.Top:SetPoint("TOP", SS, "TOP", 0, E.Border)
			SS.Bottom:SetPoint("BOTTOM", SS, "BOTTOM", 0, - E.Border)
			SS.Top:SetAlpha(0)
