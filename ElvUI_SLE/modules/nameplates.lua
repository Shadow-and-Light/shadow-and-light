...
function N:CreateThreatIndicator(nameplate)
	...
	nameplate.SLE_threatInfo:FontTemplate(E.db.sle.nameplates.threat.font, E.db.sle.nameplates.threat.size, E.db.sle.nameplates.threat.fontOutline)
end

hooksecurefunc(NP, 'ThreatIndicator_PostUpdate', function(threat, unit, status)
		...
		if E.db.sle.nameplates.threat.enable and threat.__owner.frameType == 'ENEMY_NPC' then
			...
			if unit and not UnitIsPlayer(unit) and UnitCanAttack('player', unit) then
				...
				if (E:NotSecretValue(status) and percent) then
					threat.__owner.SLE_threatInfo:SetFormattedText('|cffFFFFFF%.0f%%|r', percent or '')
				end
			end
		else
...
function N:CreateTargetCounter(nameplate)
	...
	nameplate.SLE_targetcount:FontTemplate(E.db.sle.nameplates.targetcount.font, E.db.sle.nameplates.targetcount.size, E.db.sle.nameplates.targetcount.fontOutline)
